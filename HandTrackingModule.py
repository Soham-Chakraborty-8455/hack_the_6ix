...
class HandDetector():
    def __init__(self, mode=False, maxHands=2, modelComplexity=1, detection_confidence=0.5, tracking_confidence=0.5):
        ...
        self.hands = self.mpHands.Hands(self.mode, self.maxHands, self.modelComplexity, self.detection_confidence,
        ...
        # Part of the finger counting module
    ...
    def fingersUp(self):
        ...
        if (len(self.idList) != 0):
            ...
            # Thumb
            if (self.idList[self.tipId[0]][1] < self.idList[self.tipId[0] - 2][1]):
                fingers.append(1)
            else:
                fingers.append(0)
            # Other fingers
            for id in range(1, len(self.tipId)):
                if (self.idList[self.tipId[id]][2] < self.idList[self.tipId[id] - 2][2]):
                    fingers.append(1)
                else:
                    fingers.append(0)
